...
class NetEase(object):
    ...
    def _raw_request(self, method, endpoint, data=None):
        if method == "GET":
            resp = self.session.get(
                endpoint, params=data, headers=self.header,
                timeout=DEFAULT_TIMEOUT
            )
        elif method == "POST":
            resp = self.session.post(
                endpoint, data=data, headers=self.header,
                timeout=DEFAULT_TIMEOUT
            )
        return resp
    ...
    def fm_like(self, songid, like=True, time=25, alg="itembased"):
        ...
        params = dict(
            alg=alg, trackId=songid, like="true" if like else "false",
            time=time
        )
    ...
    def search(self, keywords, stype=1, offset=0, total="true", limit=50):
        ...
        params = dict(s=keywords, type=stype, offset=offset, total=total,
                      limit=limit)
    ...
    def playlist_detail(self, playlist_id):
        ...
        params = dict(id=playlist_id, total="true", limit=1000, n=1000,
                      offest=0)
        # cookie添加os字段
    ...
    def song_tlyric(self, music_id):
        path = "/weapi/song/lyric"
        params = dict(os="osx", id=music_id, lv=-1, kv=-1, tv=-1)
        lyric = self.request("POST", path, params).get("tlyric", {}).get(
            "lyric", [])
        if not lyric:
            return []
        else:
